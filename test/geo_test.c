...
static bool testDaylightSpan(void) {
  ...
  for (int i = 0; i < COUNTOF(gSpanTestCases); ++i) {
    ...
    for (int j = 0; j < COUNTOF(gSpanOrder); ++j) {
      ...
      if (!geo_calcDaylightSpan(testCase->pos, gSpanOrder[j], testCase->y, testCase->m, testCase->d,
                                &s, &e)) {
...
static bool testIsNight(void) {
  ...
  for (int i = 0; i < COUNTOF(gNightTestCases); ++i) {
    ...
    for (int j = 0; j < COUNTOF(gSpanOrder); ++j) {
      bool isNight = geo_isNight(testCase->pos, testCase->obsTime, gSpanOrder[j]);
...
static bool testLatLonToECEF(void) {
  ...
  for (int i = 0; i < COUNTOF(gECEFCases); ++i) {
    ...
    geo_latLonToECEF(testCase->pos, &x, &y, &z);
...
bool testSubsolarPoint(void) {
  ...
  for (int i = 0; i < COUNTOF(gSubsolarCases); ++i) {
    ...
    geo_calcSubsolarPoint(&pos, testCase->obsTime);

    if (ok && !floatsEqual(pos.lat, testCase->pos.lat, SUBSOLAR_EPSILON)) {
      fprintf(stderr, "Subsolar test case %d, Lat %f != %f\n", i, pos.lat, testCase->pos.lat);
      ok = false;
    }

    if (ok && !floatsEqual(pos.lon, testCase->pos.lon, SUBSOLAR_EPSILON)) {
      fprintf(stderr, "Subsolar test case %d, Lon %f != %f\n", i, pos.lon, testCase->pos.lon);
      ok = false;
    }
  }
